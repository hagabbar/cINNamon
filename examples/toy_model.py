...
def main():
    ...
    # I think pos are parameters and lables are data
    pos, labels = data.generate(
        labels='all',
        tot_dataset_size=2**20
    )

    # just simply renaming the colors properly. Will not use in GW case
    c = np.where(labels[:test_split])[1]
    plt.figure(figsize=(6, 6))
    plt.scatter(pos[:test_split, 0], pos[:test_split, 1], c=c, cmap='Set1', s=0.25)
    plt.xticks([])
    plt.yticks([])
    plt.savefig('/home/hunter.gabbard/public_html/ml_play_githubs/FrEIA/test_distribution.png')
    plt.close()

    # setting up the model
    ndim_tot = 16 # two times the number data dimensions?
    ndim_x = 2    # number of parameter dimensions
    ndim_y = 8    # number of data dimensions
    ...
    #         print(i_epoch, end='\t ')
            ...
            axes[0].clear()
            axes[0].scatter(x_samps[:,0], x_samps[:,1], c=pred_c, cmap='Set1', s=1., vmin=0, vmax=9)
            axes[0].axis('equal')
            axes[0].axis([-3,3,-3,3])
            axes[0].set_xticks([])
            axes[0].set_yticks([])
